...
def shiping(request):
    ...
    '''
    # Получаем первый адрес доставки
    shipping_adress = ShipingAdress.objects.filter(user=request.user).first()
...
def complete_order(request):
    ...
    if request.method == 'POST':
        ...
        match payment_type:
            case "stripe-payment":
                shipping_adress = ShipingAdress.objects.create(
                    user=request.user,
                    full_name=full_name,
                    email=email,
